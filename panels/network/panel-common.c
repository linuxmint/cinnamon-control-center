...
panel_device_to_icon_name (NMDevice *device, gboolean symbolic)
{
        ...
        case NM_DEVICE_TYPE_ETHERNET:
                ...
                if (state <= NM_DEVICE_STATE_DISCONNECTED) {
                        value = symbolic ? "xsi-network-wired-disconnected-symbolic"
                                         : "network-wired-disconnected";
                } else {
                        value = symbolic ? "xsi-network-wired-symbolic"
                                         : "network-wired";
                }
                break;
        ...
        case NM_DEVICE_TYPE_OLPC_MESH:
                value = symbolic ? "xsi-network-wireless-signal-excellent-symbolic"
                                 : "network-wireless";
                break;
        case NM_DEVICE_TYPE_MODEM:
                ...
                if ((caps & NM_DEVICE_MODEM_CAPABILITY_GSM_UMTS) ||
                    (caps & NM_DEVICE_MODEM_CAPABILITY_CDMA_EVDO)) {
                        value = symbolic ? "xsi-network-cellular-signal-excellent-symbolic"
                                         : "network-cellular";
                        break;
                }
                /* fall thru */
        default:
                value = symbolic ? "xsi-network-idle-symbolic"
                                 : "network-idle";
                break;
        }
        return value;
